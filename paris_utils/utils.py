...
from paris_utils import louvain
...
class hsbm:
    def __init__(self, numbers, intensities):
        ...
        self._numbers = numbers # number of subgraphs in different layers
        self._mus = {i: np.prod(intensities[i:]) for i in range(0, len(intensities))}
        size = int(np.prod(numbers)) # total number of subgraphs
        self._matrix = self._mus[0] * np.ones((size,size))
        for l in range(1, len(self._mus)):
            for b in range(int(np.prod(numbers[:l]))):
                size_l = size // np.prod(numbers[:l])
                # set up initial clustering adjacency matrix
